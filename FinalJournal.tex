...
Training employs AdamW optimizer with learning rate $3 \times 10^{-4}$, batch size 16, and early stopping based on validation accuracy. After 21 epochs, the multimodal model achieves 79.31\% validation accuracy on HAM10000, demonstrating effective transfer of the fusion approach to an independent dataset with different imaging protocols. Notably, the model shows improved performance on minority classes compared to image-only baselines: df F1-score increases from 0.68 to 0.76 (+11.8\%), and vasc F1-score improves from 0.71 to 0.79 (+11.3\%). These gains confirm that metadata integration provides particularly strong benefits for rare lesion types where visual appearance alone may be ambiguous.
